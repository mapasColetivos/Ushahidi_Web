	...
	function Timeline(options) {
		...
		*/
		this.addMarkers = function(startDate,endDate, currZoom, currCenter,
			thisLayerID, thisLayerType, thisLayerUrl, thisLayerColor, json_url,keep_markers)
		{
			...
			if ((keep_markers === undefined) && markers && markers.length > 0)
			{
            ...
            markers.events.register("loadend", markers, function() {
				apply_filters();			
            });
...
//			{ // Keep the Base Layer in Focus
				selectControl = new OpenLayers.Control.SelectFeature(markers,{hover:true, clickout:true});
