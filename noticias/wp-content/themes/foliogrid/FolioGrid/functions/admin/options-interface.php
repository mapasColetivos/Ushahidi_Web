...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case 'textarea':
			...
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="8">' . stripslashes($val). '</textarea>';
		break;
		...
		case 'typography_r':	
			...
			$faces = of_recognized_font_faces_reduced();
			foreach ( $faces as $key => $face ) {
				$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_stored['face'], $key, false ) . '>' . esc_html( $face ) . '</option>';
			}			
