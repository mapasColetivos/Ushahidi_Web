...
class Login_Controller extends Template_Controller {
    ...
    public function index()
    {
        ...
        if ($auth->logged_in() OR $auth->auto_login())
        {
            if ($user = Session::instance()->get('auth_user',FALSE))
            {
				...
				url::redirect(url::site().'users/index/'.$user->id);
            }
        }
        ...
        if ($_POST->validate())
        {
        	...
        	if($_POST["oauth_enabled"] != 0){
        		$name = $_POST["oauth_name"];
        		$username = $_POST["oauth_username"];
        		$email = $_POST["oauth_email"];
        		$id = $_POST["oauth_enabled"];
        		...
        		if ($user->id == 0){
        			...
        			$user->save();
        			ORM::factory("roles_user")->insert_this_role($user->id,1);
					    $auth->force_login($user);
					    url::redirect(url::site().'users/index/'.$user->id);
        		} else {
        			if ($user->oauth_uid == $_POST["oauth_enabled"] && $user->username = $_POST["oauth_email"]){
						    $auth->force_login($user);
						    url::redirect(url::site().'users/index/'.$user->id);
        			} else {
                $_POST->add_error('facebook', 'login error');
        			}
        		}
			} else {
	    ...
	    if ($_POST)
	    {
			...
			if ($post->validate())
	    	{
				...
				if ($user->loaded==true)
				{
					...
					$details_sent = $this->_email_resetlink($post->resetemail,$user->name,$secret_link,$user->username );
					if( $details_sent )
					{
						$password_reset = TRUE;
					}		
	 ...
	 * @return void.
	 */
	public function _email_resetlink( $email, $name, $secret_url, $user_name )
	{
		$to = $email;
		...
		$message .= Kohana::lang('ui_admin.password_reset_message_line_2').' '.$user_name.". ";
