...
class Reports_Controller extends Main_Controller {
	...
	public function index($cluster_id = 0)
	{
		...
		} else if (isset($_GET['t']) AND !empty($_GET['t']) AND $_GET['t']!=0){
			$tag_id = $db->escape($_GET['t']);
			$query = 'SELECT it.incident_id AS incident_id FROM '.$this->table_prefix.'incidents_tags AS it INNER JOIN '.$this->table_prefix.'tags AS t ON (it.tag_id = t.id)  WHERE t.id='.$tag_id.';';
			$query = $db->query($query);
		...
		$this->themes->js->incidents = $incidents;
		...
		if ($_POST)
		{
			...
			if ($post->validate())
			{
				...
				if ($id){
					...
					$incident->user_id = 0;	
					$incident->owner_id = $this->user->id;
					...
					$layer = new Location_Layer_Model();
					$layer->incident_id = $incident->id;
					$layer->layer_color = "ff0000";
					$layer->layer_name = Kohana::lang('ui_main.layer_default_name');
					$layer->save();					
				}
				
				$incident->incident_title = $post->incident_title;
				$incident->incident_description = $post->incident_description;
				$incident->incident_default_zoom = $post->default_zoom;
				$incident->incident_default_lat = $post->default_lat;
				$incident->incident_default_lon = $post->default_lon;
				$incident->incident_privacy = $_POST["incident_privacy"];				
				$incident->save();


				// STEP 3: SAVE CATEGORIES
				ORM::factory('incident_category')->where('incident_id',$id)->delete_all();
				...
				url::redirect('index.php/locations/submit/'.$incident->id);
			}
	...
	public function edit($id = false, $saved = false)
	{
      ...
      url::redirect('login');
		...
		(
			...
			'incident_minute' => '',
			'incident_ampm' => '',
			'latitude' => $incident->incident_default_lat,
			'longitude' => $incident->incident_default_lon,
			'location_name' => '',
			'country_id' => '',
			'incident_category' => array(),
			'incident_news' => array(),
			...
			'points' => array(),
			'tags' => '',
			'default_zoom' => $incident->incident_default_zoom,
			'default_lat' => $incident->incident_default_lat,
			'default_lon' => $incident->incident_default_lon,
			'incident_privacy' => $incident->incident_privacy,
		);
		...
		$this->themes->js->default_lon = $form['default_lon'];
        $this->themes->js->default_lat = $form['default_lat'];                	
		$this->themes->js->category = ORM::factory('incident_category')->where("incident_id",$id)->find()->category_id;
	...
	public function view($map_id = false){
		...
		$this->themes->js->color_with_category = false;
		
		$this->themes->js->markers = $incident->locations;
		$this->themes->js->incident = $incident;
		$this->themes->js->layers = $incident->layers();		
		
		$this->template->header->fb_title = $incident->incident_title;
		$this->template->header->fb_description = $incident->incident_description;
		$this->template->header->fb_image = url::base()."media/images/logo_globo_guia.png";
	...
	public function view_ushahidi($id = false)
	{
		...
		if ( !$id )
		{
			...
			$this->template->content->incident_location = $incident->locations[0]->location_name;
			$this->template->content->incident_latitude = $incident->locations[0]->latitude;
			$this->template->content->incident_longitude = $incident->locations[0]->longitude;
			...
			foreach($incident->media as $media)
			{
				...
				elseif ($media->media_type == 1) //PHOTO
				{
		...
		$this->themes->js->default_map = Kohana::config('settings.default_map');
		$this->themes->js->default_zoom = Kohana::config('settings.default_zoom');
		$this->themes->js->latitude = $incident->locations[0]->latitude;
		$this->themes->js->longitude = $incident->locations[0]->longitude;
	...
	{	
        ...
        ((ACOS(SIN($latitude * PI() / 180) * SIN(l.`latitude` * PI() / 180) + COS($latitude * PI() / 180) * COS(l.`latitude` * PI() / 180) * COS(($longitude - l.`longitude`) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS distance
         FROM `".$this->table_prefix."incident` AS i INNER JOIN `".$this->table_prefix."location` AS l ON (i.`id` = l.`incident_id`) INNER JOIN `".$this->table_prefix."incident_category` AS ic ON (i.`id` = ic.`incident_id`) INNER JOIN `".$this->table_prefix."category` AS c ON (ic.`category_id` = c.`id`) WHERE i.incident_active=1
