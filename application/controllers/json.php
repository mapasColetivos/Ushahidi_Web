...
class Json_Controller extends Template_Controller
{
    ...
    function index()
    {
        ...
        if (is_numeric($category_id) AND $category_id != '0')
        {
            ...
            $markers = ORM::factory('incident')
                                    ...
                                    ->where('incident.incident_active = 1 AND incident.incident_privacy = 0 AND ('.$this->table_prefix.'incident_category.category_id = ' . $category_id . ' ' . $where_child . ')' . $where_text)
                                    ->find_all();
        ...
        else
        {
            ...
            $markers = ORM::factory('incident')
                                    ...
                                    ->with('location')
                                    ->join('media', 'incident.id', 'media.incident_id','LEFT')
                                    ->where('incident.incident_active = 1 AND incident.incident_privacy = 0'.$where_text)
                                    ->find_all();
        }
        ...
        foreach ($markers as $marker)
          ...
          foreach($marker->locations as $location)
          {
              ...
              $json_item .= "\"color\": \"".$location->category_color()."\", \n";
              $json_item .= "\"icon\": \"".$icon."\", \n";
              $json_item .= "\"timestamp\": \"" . strtotime($marker->incident_date) . "\"";
              $json_item .= "},";
              $json_item .= "\"geometry\": {";
              $json_item .= "\"type\":\"Point\", ";
              $json_item .= "\"coordinates\":[" . $location->longitude . ", " .$location->latitude . "]";
              $json_item .= "}";
              $json_item .= "}";
    ...
    public function cluster()
    {
        ...
        $incidents_result = $db->query('SELECT i.id AS id, i.incident_title AS incident_title, i.location_id as location_id FROM '.$this->table_prefix.'incident AS i WHERE i.incident_active = 1 AND i.incident_privacy = 0 '.$filter.' ORDER BY i.id ASC');
    ...
    public function single($incident_id = 0)
    {
        ...
        if ($marker->loaded)
        {
            ...
            $latitude = $marker->locations[0]->latitude;
            $longitude = $marker->locations[0]->longitude;
            ...
            $json_item .= "\"geometry\": {";
            $json_item .= "\"type\":\"Point\", ";
            $json_item .= "\"coordinates\":[" . $marker->locations[0]->longitude . ", " . $marker->locations[0]->latitude . "]";
            $json_item .= "}";
            $json_item .= "}";
    ...
    public function timeline( $category_id = 0 )
    {
        ...
        $query = 'SELECT UNIX_TIMESTAMP('.$select_date_text.') AS time, COUNT(id) AS number FROM '.$this->table_prefix.'incident WHERE incident_active = 1 AND incident_privacy = 0 '.$incident_id_in.' GROUP BY '.$groupby_date_text;
        $query = $db->query($query);
