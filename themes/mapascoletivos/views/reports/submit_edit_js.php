...
jQuery(window).load(function() {
	...
	// Trigger a zoom change on the map when a user selects
	// a diffrent zoom level
	$("#default_zoom").change(function(e) {
		// Get the new zoom level
		var zoomLevel = parseInt($(this).val()) + 3,
			mapCenter = new OpenLayers.LonLat($("#default_lon").val(), $("#default_lat").val());
		mapCenter.transform(proj_4326, map.getProjectionObject());

		// Zoom the map to this value
...
function endDrag(feature, pixel) {
	...
	$("#default_lat").val(latitude);
	$("#default_lon").val(longitude);
